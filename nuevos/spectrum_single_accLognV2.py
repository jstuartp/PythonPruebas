...
# Factor de escala (ej. 1.0 si ya está en cm/s2, 980 si está en g)
...
def procesar_un_sismo(archivo, mapa_info, curvas_diseno, carpeta_salida):
    """
    Procesa un archivo mseed y genera dos gráficas (con y sin diseño).
    """
    ...
    try:
        ...
        # 1. Gráfica CON espectro de diseño (Sufijo _CSCR)
        crear_grafica(
            PERIODOS, resp_x, resp_y, rot_resp,
            estacion_nombre, nombre_archivo,
            carpeta_salida,
            datos_extra=datos_curva,
            etiqueta_extra=etiqueta,
            sufijo_archivo="_CSCR" # <--- Sufijo distintivo
        )

        # 2. Gráfica SIN espectro de diseño (Solo sismo)
        crear_grafica(
            PERIODOS, resp_x, resp_y, rot_resp,
            estacion_nombre, nombre_archivo,
            carpeta_salida,
            datos_extra=None,      # <--- Forzamos None para no graficar diseño
            etiqueta_extra=None,
            sufijo_archivo=""      # <--- Sin sufijo extra (nombre estándar)
        )
...
def crear_grafica(periodos, resp_x, resp_y, rot_resp, station, filename_orig, carpeta_out,
                  datos_extra=None, etiqueta_extra=None, sufijo_archivo=""):
    """
    Crea y guarda la gráfica.
    'sufijo_archivo' permite diferenciar los nombres (ej. '_CSCR').
    """
    ...
    # Solo graficamos la curva negra si se pasan datos
    if datos_extra is not None:
        ...
        # Caso especial: Queríamos graficar pero no había datos (ej. archivo faltante), mostramos en leyenda
        # Nota: Si ambos son None (segunda gráfica), esto no se ejecuta.
    ...
    # Guardar en la carpeta dinámica con el sufijo correspondiente
    nombre_limpio = os.path.splitext(filename_orig)[0]
    # Se agrega sufijo_archivo al nombre
    ruta_guardado = os.path.join(carpeta_out, f"{nombre_limpio}_RotD100_loglog{sufijo_archivo}.png")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("   Uso: python script.py <nombre_evento>")
        sys.exit(1)
    ...
    print(f"\n--- Iniciando procesamiento secuencial de {total} archivos ---")

    # 4. Ejecución Secuencial (Ciclo FOR simple)
    resultados = []
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        res = procesar_un_sismo(archivo, mapa_info, curvas_diseno, carpeta_salida)
