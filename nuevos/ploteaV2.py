...
def Plotear(imagenpng, ruta, t0_str):
    """Plot seismic traces applying instrument response and trimming around t0."""
    # client = Client(FDSN_HOST)

    # Convertir el string de tiempo a objeto UTCDateTime
    try:
        t0 = UTCDateTime(t0_str)
        inicio_recorte = t0 - 30.0
        fin_recorte = t0 + 30.0
    except Exception as e:
        print(f"Error al procesar el tiempo t0: {e}")
        return
    ...
    for archivo in archivos_mseed:
        ...
        # ==============================================================================
        # NUEVA FUNCIONALIDAD: Recorte de señal (70s antes y 70s después de t0)
        # ==============================================================================
        logging.info(f"Recortando traza desde {inicio_recorte} hasta {fin_recorte}")
        strNew.trim(starttime=inicio_recorte, endtime=fin_recorte)

        # Verificamos si la traza quedó vacía tras el recorte (por si el mseed no contiene el t0)
        if not strNew or len(strNew) == 0:
            logging.warning(f"La traza {nombre_imagen} quedó vacía tras el recorte. Saltando.")
            continue
        # ==============================================================================
        ...
        # Se verifica nuevamente len(strNew) por seguridad antes de plotear
        if len(strNew) == 0:
            continue
...
def main():
    ...
    # Nuevo argumento t0
    parser.add_argument("--t0", type=str, required=True, help="Tiempo origen t0 (ISO string, ej: 2026-01-01T12:00:00)")

    args = parser.parse_args()

    # Pasamos el t0 a la función
    Plotear(args.imagenpng, args.ruta, args.t0)
