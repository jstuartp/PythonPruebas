...
def estaciones_con_HN(inventory) -> List[Tuple[str, str, str]]:
    ...
    for net in inventory.networks:
        for sta in net.stations:
            ...
            # 1. Comprobación original: Los 3 canales en el MISMO location code
            for loc, cods in chans_por_loc.items():
                if {"HNE", "HNN", "HNZ"}.issubset(cods):
                    triples.add((net.code, sta.code, loc))

            # 2. Nueva comprobación: Equipo dividido en locs 11, 12 y 13
            # Extraemos los sets de cada loc (usando set() vacío si el loc no existe)
            if ("HNZ" in chans_por_loc.get("11", set()) and
                    "HNN" in chans_por_loc.get("12", set()) and
                    "HNE" in chans_por_loc.get("13", set())):
                # Agregamos una tupla con un identificador especial para el location
                triples.add((net.code, sta.code, "11-12-13"))
...
def leer_y_convertir_a_acc_cmss(client: SDSClient, inv, net: str, sta: str, loc: str,
    ...
    # Manejo del caso especial donde los canales están en diferentes location codes
    if loc == "11-12-13":
        # Extraemos cada canal individualmente y los sumamos al Stream
        st += client.get_waveforms(net, sta, "11", "HNZ", t0, t1, merge=1)
        st += client.get_waveforms(net, sta, "12", "HNN", t0, t1, merge=1)
        st += client.get_waveforms(net, sta, "13", "HNE", t0, t1, merge=1)
    else:
        # Manejo normal
...
def main():
    ...
    logging.info(f"FECHA FORMATO {args.start}")
    out_dir = OUT_DIR
    inv_path = Inventory
    ...
    for net, sta, loc in triples:
        ...
        # También replicamos la lógica para stRaw por si se descomenta en el futuro
        stRaw = Stream()
        if loc == "11-12-13":
            stRaw += client.get_waveforms(net, sta, "11", "HNZ", t0, t1, merge=1)
            stRaw += client.get_waveforms(net, sta, "12", "HNN", t0, t1, merge=1)
            stRaw += client.get_waveforms(net, sta, "13", "HNE", t0, t1, merge=1)
        else:
            loc_sel = loc if loc else "*"
    ...
    resultCopia = subprocess.run(
        ['rsync', '-av', "/home/lis/waves/jsons/" + args.event + "/edificio", WEB_SERVER + "/" + args.event + "/"])
