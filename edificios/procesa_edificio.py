    ...
    for (net, sta), st in agrupados.items():
        ...
        try:
            ...
            for tr in st:
                ...
                pgvsChannel.append({str(tr.stats.channel).lower()+'_vel': pgv})
                pgdsChannel.append({str(tr.stats.channel).lower()+'_des': pgd})
            maximos.append({
                "station": station,
                "maximos": m
            })
            ...
            for i in pgasChannel:
                ultimo.update(i)
            for x in pgvsChannel:
                ultimo.update(x)
            for y in pgdsChannel:
                ultimo.update(y)
            streams.append({
                "evento": evento,
                "network": network,
        ...
        #print(resultados)
    #raise ValueError("Terminando debuggggg")
...
def insertaBd(datos):
    ...
    try:
        valores = [datos['fecha_evento'], datos['fecha_calculo'], datos['evento'], datos['tipo'], datos['estacion'],
                   ...
                   datos['network'] + "_" + datos['estacion'] + "_" + datos[
        ...
        try:
            with (conn.cursor() as cursor):
                ...
                sql = (
                    ...
                    " `longitud`=%s, `hne_pga`=%s, `hnn_pga`=%s, `hnz_pga`=%s, `hne_pgv`=%s, `hnn_pgv`=%s, `hnz_pgv`=%s,"
